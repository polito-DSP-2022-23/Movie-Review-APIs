    ...
    return new Promise((resolve, reject) => {
        ...
        db.all(sql1, [filmId], (err, rows) => {
            if (err)
              ...
              var sql3 = `UPDATE films SET `; 
              var parameters=[]; 
              if(Object.keys(film).length==1){
                sql3+=`${Object.keys(film)[0]} = ?`; 
                parameters.push(film[Object.keys(film)[0]])
              }else{
 ...
 **/
 exports.getInvitedFilms = function(req) {
    return new Promise((resolve, reject) => {
      ...
      var sql = "SELECT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite FROM films f, reviews r, reviewers rs WHERE f.private = 0 AND f.id = r.filmId AND r.reviewId = rs.reviewId AND rs.reviewerId = ?"
      var limits = getPagination(req);
      if (limits.length != 0) sql = sql + " LIMIT ?,?";
      ...
      db.all(sql, limits, (err, rows) => {
          if (err) {
              reject(err);
                ...
                let films = uniqueRows.map((row) => createFilm(row));
                resolve(films);
          }
      });
    });
 ...
 **/
 exports.getInvitedFilmsTotal = function(reviewerId) {
    return new Promise((resolve, reject) => {
        var sql = "SELECT f.title FROM films f, reviews r, reviewers rs WHERE  f.private = 0 AND f.id = r.filmId AND r.reviewId=rs.reviewId AND rs.reviewerId=?";
        db.all(sql, [reviewerId], (err, rows) => {
            if (err) {
                ...
                let length=0; 
                for(let i=0; i<rows.length; i++){
                    if(!(lastTitle.includes(rows[i].title))){
                        length=length+1; 
                        lastTitle.push(rows[i].title);
                    }
                }
                resolve(length);
            }
